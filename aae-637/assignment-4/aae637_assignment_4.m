...
fluid_inc_elast_val = normpdf(mean(mod_data(:, 2:end))*fluid_betas) * ...
  (fluid_betas(3) + fluid_betas(4)*mean(mod_data(:, 3)) ) * ...
  (mean(mod_data(:, 4)) / normcdf(mean(mod_data(:, 2:end))*fluid_betas))
  % 4 is income 
  % 3 is num_young
...
mean_ch_2_50 = mean_ch_2;
mean_ch_3_50 = mean_ch_3;
...
mean_ch_2_150 = mean_ch_2;
mean_ch_3_150 = mean_ch_3;
...
mod_data = horzcat( ...
    full_data(:, strcmp(varnames,'Majordrg')) > 0, ...
    repmat(1, size(full_data, 1), 1), ...
    full_data(:, strcmp(varnames,'Age')), ...
   (full_data(:, strcmp(varnames,'Age'))).^2, ...
    full_data(:, strcmp(varnames,'Inc_per')), ...
    full_data(:, strcmp(varnames,'Avgexp')), ...
    full_data(:, strcmp(varnames,'Ownrent')) ...
    );
...
age_elast_credit = logitcdf_fn(mean(mod_data(:, 2:end)) * credit_betas) * ...
  ( 1 - logitcdf_fn(mean(mod_data(:, 2:end)) * credit_betas)) * ...
  (credit_betas(2) + 2*credit_betas(3)*mean(mod_data(:, 3)) ) * ...
  mean(mod_data(:, 3)) / logitcdf_fn(mean(mod_data(:, 2:end)) * credit_betas)
% P. 729 of Greene

inc_elast_credit = logitcdf_fn(mean(mod_data(:, 2:end)) * credit_betas) * ...
  ( 1 - logitcdf_fn(mean(mod_data(:, 2:end)) * credit_betas) ) * ...
  credit_betas(4)  * ...
  mean(mod_data(:, 5)) / logitcdf_fn(mean(mod_data(:, 2:end)) * credit_betas)
...
inc_elast_credit_75 = logitcdf_fn(inc_elast_data_input * credit_betas) * ...
  ( 1 - logitcdf_fn(inc_elast_data_input * credit_betas) ) * ...
  credit_betas(4)  * ...
  inc_elast_data_input(4) / logitcdf_fn(inc_elast_data_input * credit_betas)
  
inc_elast_data_input = mean(mod_data(:, 2:end));
inc_elast_data_input(4) = inc_elast_data_input(4) * 1.75;

inc_elast_credit_125 = logitcdf_fn(inc_elast_data_input * credit_betas) * ...
  ( 1 - logitcdf_fn(inc_elast_data_input * credit_betas) ) * ...
  credit_betas(4)  * ...
  inc_elast_data_input(4) / logitcdf_fn(inc_elast_data_input * credit_betas)
