...
hogar01.df.temp<-hogar01.df[, c("FOLIO", "S815D2", "S815B2", "S1036", "S1037", "PERSAGRO", "ID05", "ID06", "ID01", "X", "Y", "CAT_LOC", "num.of.localidades", "comunidad.id")]
names(hogar01.df.temp)<-c("FOLIO", "fert.exp", "seed.exp", "received.credit", "credit.source", "num.pers.agropecuaria", "city", "canton", "department", "X", "Y", "CAT_LOC", "num.of.localidades", "comunidad.id")
...
names(drive.time.data.ls)<-villages.df$comunidad.id

gather.mapquest<-TRUE

for (village in villages.df$comunidad.id) {

  if(!gather.mapquest) {break}

  target.village<-villages.df[villages.df$comunidad.id==village, c("X", "Y")]
  time.calcs.final.cols <- c()

  for (settlement.type in list(c("0-Capital", "1-Urbana"), "2-Amanzanada")) {
  
    if ( pob.shp.rev$CAT_LOC[pob.shp.rev$comunidad.id==village] %in% settlement.type ) {
      time.calcs.final.cols <- c(time.calcs.final.cols, 0)
      next
    }
    
  
    urban.coords<-pob.shp[pob.shp$CAT_LOC %in% settlement.type, c("X", "Y")]
...
#    for (coord.chunk in list(1:24, 25:48, 49:72, 73:96)) {
  ...
  drive.time.data.ls[[village]]<-data.frame(comunidad.id=village, 
    drive.time.urban=time.calcs.final.cols[1], drive.time.amanzanada=time.calcs.final.cols[2],
    stringsAsFactors=FALSE)
    
  cat(which(villages.df$comunidad.id==village), date(), content(post.receipt)$info$messages, sep="\n")
...
crop.wide.df<-merge(crop.wide.df, drive.time.df, all.x=TRUE)


save(crop.wide.df, file=paste0(work.dir, "crop wide df.Rdata"))
...
fmla <- as.formula(paste("fert.exp ~ ", 
	...
	"+ indig.prop + seed.exp + credit.source + department + hhh.edu.measure + hhh.sex + REMPAIS + REMEXT + num.pers.agropecuaria + AWC.CLASS + T.GRAVEL + T.SILT + T.CLAY + T.BULK.DENSITY + T.OC + T.PH.H2O + T.CEC.CLAY + T.CEC.SOIL + T.TEB + T.CACO3 + T.CASO4 + T.ESP + T.ECE + elevation + drive.time.amanzanada + drive.time.urban"))
...
for ( i in vars.to.scale) {
  while (max(crop.wide.df.for.model[, i], na.rm=TRUE)>10) {
...
area.expl.tob2 <- censReg(fmla, data = crop.wide.df.for.model, iterlim = 1500, print.level=3))
area.expl.tob3 <- censReg(fmla, data = crop.wide.df, iterlim = 5000, print.level=3, method="BFGS")
