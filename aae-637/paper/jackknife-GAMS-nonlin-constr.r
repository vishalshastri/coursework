...
for ( i in 1:N) {
  ...
  input.scaling  <- log10_ceiling(
    ...
    combined.df[, paste0("w", lead.zero(i))])^2)/(nrow(combined.df)))) # subtle change to not ( -1)
                                           # So that it's consistent with other data
  )
...
GAMS.linear.results<- readLines(paste0(GAMS.projdir, "GMEnonlinear", strsplit(target.crop, " ")[[1]][1], 
   formatC(0, width = 5, flag = "0"), ".lst"))
...
param.gather.regex<- paste0("(^1)|", paste0("(   ", 2:length(theta.param.support), " )", collapse="|"))
...
for ( i in 1:length(prob.names)) {
  ...
  if ( grepl("theta", prob.names[i]) ) {

  start.vals.lines <- c( start.vals.lines, 
    paste0( prob.names[i], "(\"", 1:length(theta.param.support), "\") = ",  
...
end.err.weight  <- grep(paste0("^", nrow(combined.df) + 1, " "), GAMS.linear.results)
# a change to  + 1 so that we capture whole thing
...
for ( i in 1:length(all.eqns) ) {
  ...
  err.weight.temp.df <- read.table( paste0(GAMS.projdir, "GMEnonlinear", strsplit(target.crop, " ")[[1]][1], 
   formatC(0, width = 5, flag = "0"), ".lst"), 
    skip = begin.err.weight[i] + 3,  nrows= nrow(combined.df) + 1)
# a change to  + 1 so that we capture whole thing
  err.weight.temp.df <- err.weight.temp.df[ bootstrap.selection.v,]
...
cat(completed.GAMS.file, 
  file=paste0(GAMS.projdir, "GMEnonlinearjackknife", strsplit(target.crop, " ")[[1]][1], 
